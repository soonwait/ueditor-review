...
um.ready(function () {
  ...
  // 
  // ................未完
  um.setContent('<p>中共中央总书记</p><p><span contenteditable="false">\u21B5</span></p><p>中共中央总书记</p>');
  return
  // 增加了自制换行符的实现
  // 有个问题：从这行首往上一行删除时,会把这行尾换行符删掉
  // 有个问题：从这行尾往下一行删除时,会把下行尾换行符换到下下行去
  // 有个问题：从这行尾往前删除时,会多出一个换行符来(下一行换行符正常的时候)
  // 有个问题：从这行尾往前删除时,会删除掉换行符(下一行换行符有删除线时)
  // fixed 有个问题：在全文尾巴上回车时,报错
  um.setContent('<p>中共中央总书记<span class="br"><br></span></p><p>中共中央总书记<span class="br"><br></span></p><p>中共中央总书记</p>');
return;
// 2.1 单行单插入标签
...
// return;
um.setContent('<p>中共<ins cite="anonymous" datetime="2019-04-18 18:37">中央</ins>总书记</p>\
<div data-review-meta="true"><span user="veiky" color="red"></span></div>');
// 2.2 单行单删除标签
